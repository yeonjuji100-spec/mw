...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const stoneData = {
        ...
        'about': {
            ...
            items: [
                ...
                {
                    image: 'assassinationclassroom.webp',
                    title: '암살교실',
                ...
                },
                {
                    image: 'conan.webp',
                    title: '명탐정 코난',
                    synopsis: '검은 조직의 음모로 어린아이가 되어버린 고등학생 명탐정 남도일(쿠도 신이치)이 "코난"이라는 가명으로 수많은 난제와 살인 사건들을 명석한 두뇌로 해결해나가는 국민 추리 애니메이션입니다. 일상 속 소소한 코미디와 거대한 조직을 쫓는 미스터리가 어우러져 오랜 시간 큰 사랑을 받고 있습니다.',
                    tags: ['#추리', '#미스터리', '#일상', '#코미디', '#탐정', '#검은조직'],
                    genres: '추리·일상·코미디',
                    medium: 'TVA',
                    age: '15',
                    rating: '4.8',
                ...
                },
                {
                    image: 'shinchan.webp',
                    title: '짱구는 못말려',
                    synopsis: '떡잎마을에 사는 어디로 튈지 모르는 5살 유치원생 신짱구와 짱구네 가족들, 그리고 유치원 친구들의 시끌벅적하고 유쾌한 일상을 담은 국민 코미디 애니메이션. 때로는 배꼽 빠지는 웃음을, 때로는 가족의 따뜻한 감동을 선사합니다.',
                    tags: ['#코미디', '#일상', '#가족물', '#유치원', '#어린이', '#국민애니'],
                    genres: '코미디·일상·가족',
                    medium: 'TVA',
                    age: '전체',
                    rating: '4.7',
        ...
        'departments': {
            ...
            items: [
                ...
                },
                {
                    image: 'ippo.webp',
                    title: '더 파이팅',
                    synopsis: '어머니와 단둘이 낚시 배 대여점을 운영하며, 학교에서는 괴롭힘을 당하던 내성적인 소년 잇포. 어느 날 유명 복서 타카무라 마모루의 도움을 받게 되고, 강해지고 싶다는 열망으로 복싱을 시작하게 됩니다. 수많은 강적들과의 치열한 시합을 통해 진정한 강함이 무엇인지 깨달아가는 열혈 스포츠 복싱 애니메이션의 전설입니다.',
                    tags: ['#스포츠', '#복싱', '#성장물', '#열혈물', '#노력', '#우정'],
                    genres: '스포츠·복싱·성장',
                    medium: 'TVA',
                    age: '15',
                    rating: '4.8',
                ...
                {
                    image: 'yowamushi.webp',
                    title: '겁쟁이 페달',
                    synopsis: '애니메이션을 너무나 사랑하는 소년 오노다 사카미치. 매주 아키하바라까지 왕복 90km 거리를 무거운 자전거로 다니던 그의 엄청난 페달링 잠재력을 알아본 동급생의 권유로 소호쿠 고교 자전거 경기부에 입부하게 됩니다. 개성 넘치는 선배, 동기들과 함께 전국체전 우승을 향해 땀 흘리며 페달을 밟는 본격 로드레이스 청춘물입니다.',
                    tags: ['#스포츠', '#자전거', '#청춘', '#성장물', '#팀워크', '#열혈물'],
                    genres: '스포츠·청춘·성장',
                    medium: 'TVA',
                    age: '12',
                    rating: '4.7',
        ...
        'portfolio': {
            ...
            items: [
                ...
                },
                {
                    ...
                    title: '사이버펑크: 엣지러너',
                    synopsis: '신체 개조가 지배하는 디스토피아 미래 도시 나이트 시티. 모든 것을 잃은 빈민가 소년 데이비드는 생존을 위해 위험한 신체 개조를 감행하고 용병인 \'엣지러너\'가 됩니다. 우연히 만난 네트러너 소녀 루시와 운명적인 사랑에 빠진 그는, 오직 그녀의 꿈을 지켜주기 위해 목숨을 건 처절한 질주를 멈추지 않습니다. 강렬한 액션과 잊을 수 없는 애절한 로맨스로 여운을 남기는 명작입니다.',
                    tags: ['#로맨스', '#SF', '#사이버펑크', '#비극', '#액션', '#청춘', '#명작'],
                    genres: '로맨스·SF·액션',
                    medium: 'TVA',
                    age: '19',
                    rating: '4.8',
                    rankBadge: '올해의 애니메이션 수상작'
                }
            ]
        },
        'default': {
    ...
    // 🚀 성능 최적화: 초기 렌더링을 방해하지 않도록 페이지 로드 1초 후 백그라운드에서 모든 갤러리 이미지를 미리 캐싱 (Preload)
    setTimeout(() => {
        const preloadedImages = new Set();
        for (const key in stoneData) {
            if (stoneData.hasOwnProperty(key)) {
                stoneData[key].items.forEach(item => {
        ...
        console.log('Preloaded gallery images in background for instant transitions.');
    }, 1000);
    ...
    document.addEventListener('keydown', (e) => {
        ...
        const randomOpen  = document.getElementById('random-overlay') && document.getElementById('random-overlay').classList.contains('active');

        if (e.key === 'Escape') {
            // 우선순위: 정보 모달 → 갤러리 → 게시판 → 랜덤뽑기
            if (infoOpen) {
    ...
    async function showDetailView(postId) {
        ...
        document.getElementById('detail-views').textContent   = '👁️ ' + (post.views || 0);
    ...
    async function renderComments(postId) {
        ...
        commentsListEl.innerHTML = '<div style="color: rgba(255,255,255,0.5); padding: 10px 0;">댓글을 불러오는 중...</div>';
        try {
            ...
            if (snapshot.empty) {
                commentsListEl.innerHTML = '<div style="color: rgba(255,255,255,0.4); padding: 10px 0; font-size: 0.9rem;">아직 작성된 댓글이 없습니다. 첫 댓글을 남겨보세요!</div>';
                return;
            }
            ...
            commentsData.forEach(data => {
                ...
                div.innerHTML = `
                    <div class="comment-header">
                        ...
                        <div>
                            ...
                            <span class="comment-delete-btn" title="댓글 삭제">🗑️</span>
                        </div>
                    </div>
                ...
                deleteBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    if (data.password) {
                        const inputPw = prompt('댓글을 삭제하려면 비밀번호를 입력해주세요.');
                        ...
                        if (!confirm('이 댓글을 삭제하시겠습니까? (비밀번호 없음)')) return;
                    }
                    try {
                        await db.collection("comments").doc(data.id).delete();
                        await renderComments(postId);
                    } catch (err) {
                        console.error(err);
                        alert('댓글 삭제에 실패했습니다.');
            ...
            commentsListEl.innerHTML = '<div style="color: rgba(255,100,100,0.8); padding: 10px 0;">댓글을 불러오는 데 실패했습니다.</div>';
        }
    }
        ...
        try {
            ...
            currentPosts.forEach((post, idx) => {
                ...
                item.innerHTML = `
                    ...
                    <span class="post-views-cell">${post.views || 0}</span>`;
    ...
    submitPostBtn.addEventListener('click', async (e) => {
        ...
        const password = document.getElementById('post-password-input').value.trim();
    ...
    deletePostBtn.addEventListener('click', async (e) => {
        ...
        if (post && post.password) {
            const inputPw = prompt('이 게시글을 삭제하려면 비밀번호를 입력해주세요.');
            if (inputPw === null) return;
            ...
            if (!confirm('이 게시글을 삭제하시겠습니까? (비밀번호 없음)')) return;
        }
    ...
    const menuAnipick = document.getElementById('menu-anipick');
    const menuRandom = document.getElementById('menu-random');
    const menuBoard = document.getElementById('menu-board');
    const menuPopular = document.getElementById('menu-popular');
    ...
    function openAnimeByTitle(title) {
        for (const [stoneId, data] of Object.entries(stoneData)) {
            if (data.items) {
    ...
    if (menuFavorites) {
        menuFavorites.addEventListener('click', (e) => {
            e.stopPropagation();
            const dropdown = document.getElementById('favorites-list');
            ...
            // Get favorited titles from localStorage
            const favoritedTitles = Object.keys(localStorage)
                .filter(key => key.startsWith('fav_') && localStorage.getItem(key) === 'true')
                .map(key => key.replace('fav_', ''));
                
            if (favoritedTitles.length === 0) {
                dropdown.innerHTML = '<li class="favorite-post-item" style="text-align:center;">찜한 애니가 없습니다.</li>';
                return;
            }
            
            favoritedTitles.forEach(title => {
                const li = document.createElement('li');
                li.className = 'favorite-post-item';
                li.innerHTML = escapeHTML(title);
                
                li.addEventListener('click', (ev) => {
                    ev.stopPropagation();
                    dropdown.classList.remove('active');
                    openAnimeByTitle(title);
                });
    ...
    if (menuAnipick) {
        menuAnipick.addEventListener('click', (e) => {
            e.stopPropagation();
            const dropdown = document.getElementById('anipick-list');
            ...
            anipicks.forEach((title, index) => {
                const li = document.createElement('li');
                li.className = 'popular-post-item';
                li.innerHTML = `<span class="popular-post-rank">${index + 1}</span> ${escapeHTML(title)}`;
    ...
    const randomPlatform = document.getElementById('random-platform');
    const randomWatchBtn = document.getElementById('random-watch-btn');
    const gumballMachine = document.querySelector('.gumball-machine');

    if (menuRandom) {
        menuRandom.addEventListener('click', (e) => {
            e.stopPropagation();
    ...
    if (randomCloseBtn) {
        randomCloseBtn.addEventListener('click', (e) => {
            e.stopPropagation();
    ...
    if (randomStartBtn) {
        randomStartBtn.addEventListener('click', () => {
            gumballMachine.classList.add('animating');
            randomResult.classList.add('hidden');
            
            const allAnimes = [];
            for (const key in stoneData) {
                if (stoneData[key] && stoneData[key].items) {
                    allAnimes.push(...stoneData[key].items);
                }
            }

            setTimeout(() => {
                gumballMachine.classList.remove('animating');
                
                if (allAnimes.length > 0) {
                    const randomIndex = Math.floor(Math.random() * allAnimes.length);
                    const selectedAnime = allAnimes[randomIndex];
                    
                    randomTitle.textContent = selectedAnime.title;
                    
                    const platforms = ['Laftel', 'Netflix', 'Watcha', 'Wavve', 'Tving'];
                    const p1 = platforms[Math.floor(Math.random() * platforms.length)];
                    let p2 = platforms[Math.floor(Math.random() * platforms.length)];
                    while(p1 === p2) {
                        p2 = platforms[Math.floor(Math.random() * platforms.length)];
                    }
                    randomPlatform.textContent = `플랫폼: ${p1}, ${p2}`;
    ...
    if (randomWatchBtn) {
        randomWatchBtn.addEventListener('click', () => {
            randomOverlay.classList.remove('active');
            document.body.classList.remove('gallery-active');
            
            const streamingStone = document.getElementById('streaming-stone');
            if (streamingStone) {
                streamingStone.scrollIntoView({ behavior: 'smooth', block: 'center' });
                // Optional highlight effect using simple inline CSS temp transition
                const originalBg = streamingStone.style.backgroundColor;
                streamingStone.style.transition = 'background-color 0.5s';
                streamingStone.style.backgroundColor = 'rgba(255,255,255,0.2)';
                setTimeout(() => {
                    streamingStone.style.backgroundColor = originalBg;
                }, 1500);
            }
        });
    }
    ...
    if (menuPopular) {
        menuPopular.addEventListener('click', async (e) => {
            e.stopPropagation();
            const dropdown = document.getElementById('popular-posts-list');
            ...
            dropdown.classList.add('active');
            dropdown.innerHTML = '<li class="popular-post-item" style="text-align:center;">로딩 중...</li>';
            
            try {
                // 직접 조회수 상위 5개만 Firebase에서 가져오기 (매우 빠름)
                const querySnapshot = await db.collection("posts").orderBy("views", "desc").limit(5).get();
                const topPosts = [];
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    // 상세보기가 작동하도록 currentPosts 배열에 업데이트/추가
                    const existingIndex = currentPosts.findIndex(p => p.id === data.id);
                    if (existingIndex > -1) {
                        currentPosts[existingIndex] = data;
                    } else {
                ...
                if (topPosts.length === 0) {
                    dropdown.innerHTML = '<li class="popular-post-item" style="text-align:center;">게시글이 없습니다.</li>';
                    return;
                }
                ...
                topPosts.forEach((post, index) => {
                    const li = document.createElement('li');
                    li.className = 'popular-post-item';
                    li.innerHTML = `<span class="popular-post-rank">${index + 1}</span> ${escapeHTML(post.title)}`;
                    ...
                    // Open board if it's not open
                    openBoard();
                    
                    // Show details
                    setTimeout(() => {
                        showDetailView(post.id);
                    }, 50);
                });
                ...
                dropdown.innerHTML = '<li class="popular-post-item" style="text-align:center;">게시글을 불러오는데 실패했습니다.</li>';
            }
        });
    }
    ...
    if (sidebarToggle && sidebar) {
        ...
        // Close sidebar when clicking any non-dropdown item inside
        ...
        document.addEventListener('click', (e) => {
            const dropdownItem = e.target.closest('.popular-post-item, .favorite-post-item');
